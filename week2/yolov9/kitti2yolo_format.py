...
def xyhw2yolo(coordinates, dims):    
    x,y,w,h = coordinates
...
for file in sorted(os.listdir(annotations_dir)):
    ...
    for i, (frame_nr, class_nr) in enumerate(zip(frame_numbers, class_numbers)):
        ...
        if int(class_nr) != 10:
            ...
            rle = rles[i]
            size = image_dims[i]
            #bbox = [int(coord) for coord in rletools.area(decompose_rle(rle, size))]
            ##yolo_bbox = xyhw2yolo(bbox, image_dims[i])

            class_nr_adjusted = str(class_nr - 1)
            
            #ann_string = f'{class_nr_adjusted} {yolo_bbox[0]} {yolo_bbox[1]} {yolo_bbox[2]} {yolo_bbox[3]}\n'
            ann_string = f'{class_nr_adjusted} {rle}\n'
