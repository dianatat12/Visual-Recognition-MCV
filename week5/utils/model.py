...
class ImageTextRetrievalModel(pl.LightningModule):
    def __init__(
        ...
        self.image_projection = nn.Sequential(
            nn.Linear(image_embedding_dim, image_embedding_dim),
            nn.ReLU(),
            nn.BatchNorm1d(image_embedding_dim),
        )
        self.text_projection = nn.Sequential(
            nn.Linear(text_embedding_dim, image_embedding_dim // 2),
            nn.ReLU(),
            nn.Linear(image_embedding_dim // 2, image_embedding_dim),
            nn.ReLU(),
            nn.BatchNorm1d(image_embedding_dim),
        )
